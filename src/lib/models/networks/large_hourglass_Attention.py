...
class bifpn(nn.Module):
    ...
    def forward(self, low, a, low_list):
...
class kp_module(nn.Module):
    def __init__(
        ...
        self.low2 = kp_module(
            ...
            **kwargs
                ...
                bifpn_layer()
                # make_low_layer(3, next_dim, next_dim, next_mod,layer=layer, **kwargs)
    ...
    def forward(self, x, bifpn_list, low_list):
        ...
        self.low_list.append(low1)
        low2, self.low_list = self.low2(low1, [], self.low_list)
        low3 = self.low3(low2)
        up2  = self.up2(low3)

        return self.merge(self.low_list[5-self.n], up2), self.low_list

class kp_module2(nn.Module):
    def __init__(
        self, n, dims, dims_, modules, layer=residual,
        make_up_layer=make_layer, make_low_layer=make_layer,
        make_hg_layer=make_layer, make_hg_layer_revr=make_layer_revr,
        make_pool_layer=make_pool_layer, make_unpool_layer=make_unpool_layer,
        bifpn_layer = bifpn, make_merge_layer=make_merge_layer, **kwargs
        ...
        if(n==5):
            self.attention1 = Attention(dims[0])

        self.attention = Attention(dims[6-n])
        self.n_  = n
        self.n   = n
        curr_mod = modules[0]
        next_mod = modules[1]
        ...
        self.low2 = kp_module2(
            n - 1, dims[1:],dims_, modules[1:], layer=layer, 
            make_up_layer=make_up_layer, 
            make_low_layer=make_low_layer,
            make_hg_layer=make_hg_layer,
            make_hg_layer_revr=make_hg_layer_revr,
    ...
    def forward(self, x, bifpn_list, low_list):
        ...
        max1 = self.max1(x)
        if(self.n == 5):
            max1 = self.attention1(max1,bifpn_list[0])
        ...
        if(self.n==1):
            ...
            low2, _ = self.low2(self.attention(low1,bifpn_list[6-self.n]),bifpn_list,[])
...
class exkp(nn.Module):
    def __init__(
        ...
        self.kps  = nn.ModuleList([
            ...
            kp_module2(
                n, dims, dims, modules, layer=kp_layer,
                make_up_layer=make_up_layer,
                make_low_layer=make_low_layer,
                make_hg_layer=make_hg_layer,
                make_hg_layer_revr=make_hg_layer_revr,
    ...
    def forward(self, image):
        ...
        for ind in range(self.nstack):
            ...
            kp, bifpn_list  = kp_(inter, bifpn_list, [])
            cnv = cnv_(kp)
...
class Attention(nn.Module):
    def __init__(self,input_dim):
        ...
        self.query_conv = nn.Conv2d(input_dim, input_dim, kernel_size=1,stride=1)
        self.key_conv   = nn.Conv2d(input_dim, input_dim, kernel_size=1,stride=1)
        self.value_conv = nn.Conv2d(input_dim, input_dim, kernel_size=1,stride=1)

        self.softmax    = nn.Softmax(dim=-1)
        self.input_dim  = input_dim
        self.drop_attn = nn.Dropout(0)
        self.drop_soft = nn.Dropout(0)
        #self.final_conv = nn.Conv2d(64, input_dim, kernel_size=1,stride=1)
    ...
    def forward(self,x,query):
        ...
        query = query.reshape(B,C,H*W)              ## B, C, N

        key   = self.key_conv(x)                    ## B, C, H, W
        key   = key.reshape(B,C,H*W)                ## B, C, N
        value = self.value_conv(x)                  ## B, C, H, W
        value   = value.reshape(B,C,H*W)            ## B, C, N
        key   = key.permute(0,2,1)                  ## B, N, C
        
        bmm_query_key = torch.bmm(key,query)        ## N, N
        attention_map = self.softmax(bmm_query_key) ## N, N 행 방향
        self.drop_soft(attention_map)
        attention_map_T = attention_map.permute(0,2,1) ## matmul 수행하기 위해 전치

        final_attention_map = torch.bmm(value,attention_map_T)
        self.drop_attn(final_attention_map)
        final_attention_map = final_attention_map.reshape(B, C, H, W)

        out = x + final_attention_map * self.gamma
